...
import {
  buildLoreDeltaPlan,
  LoreDeltaConflict,
  LoreDeltaExistingPageInput,
  LoreDeltaPlannedChange,
  LoreDeltaPlannedPage,
  LoreDeltaProgressEvent,
  LoreDeltaResult,
  LoreDeltaUpdatePolicy
} from './lore-delta-update';
...
export const LOREVAULT_LORE_DELTA_VIEW_TYPE = 'lorevault-lore-delta-view';

type LoreDeltaConflictDecision = 'pending' | 'accept' | 'reject' | 'keep_both';
type LoreDeltaConflictFilter = 'all' | LoreDeltaConflictDecision;
type LoreDeltaConflictOutcome = 'applied' | 'rejected' | 'kept_both';

class LoreDeltaNotePickerModal extends FuzzySuggestModal<TFile> {
  private readonly files: TFile[];
  private readonly onChoose: (file: TFile) => void;

  constructor(app: App, files: TFile[], onChoose: (file: TFile) => void, placeholder: string) {
    super(app);
...
function buildConflictCompanionContent(
  page: LoreDeltaPlannedPage,
  conflict: LoreDeltaConflict,
  sourcePath: string
): string {
  ...
  const lines: string[] = [
    ...
    `loreDeltaSource: ${JSON.stringify(sourcePath || '(inline brief)')}`,
...
export class LorevaultLoreDeltaView extends ItemView {
  private plugin: LoreBookConverterPlugin;
  private sourceIdeaNotePath = '';
  private ideaMarkdown = '';
  private newNoteTargetFolder = 'LoreVault/import';
  private selectedLorebookScopes: string[] = [];
  private focusedNotePaths: string[] = [];
  private updatePolicy: LoreDeltaUpdatePolicy = 'section_merge';
  private allowCreateNotes = true;
  private defaultTags = '';
  private maxChunkChars = 5000;
  private maxOperationsPerChunk = 12;
  private maxExistingPagesInPrompt = 80;
  private maxFocusedPagesInPrompt = 6;
  private maxFocusedPageChars = 5000;
  ...
  private applyStatus = '';
  private lastPreview: LoreDeltaResult | null = null;
  private approvedPaths = new Set<string>();
  private conflictDecisions = new Map<string, LoreDeltaConflictDecision>();
  private conflictOutcomes = new Map<string, LoreDeltaConflictOutcome>();
  private processedChangePaths = new Set<string>();
  private hideProcessedConflicts = false;
  private conflictFilter: LoreDeltaConflictFilter = 'all';
  ...
  getViewType(): string {
    return LOREVAULT_LORE_DELTA_VIEW_TYPE;
  }

  getDisplayText(): string {
    return 'LoreVault Lore Delta';
  }
  ...
  private onDeltaProgress(event: LoreDeltaProgressEvent): void {
    ...
    if (event.stage === 'starting') {
      this.setProgress('Preparing lore delta preview...', event.chunkTotal ? `${event.chunkTotal} chunk(s)` : '');
      return;
    }
    if (event.stage === 'chunk_start') {
      this.setProgress('Processing lore delta chunk...', chunkLabel);
      return;
    }
    ...
    if (event.stage === 'completed') {
      ...
      this.setProgress('Lore delta preview complete', detailParts.join(' | '));
    }
  }
  ...
  private async resolveIdeaMarkdown(): Promise<string> {
    const inline = this.ideaMarkdown.trim();
    if (inline) {
      return inline;
    }

    const notePath = normalizeVaultPath(this.sourceIdeaNotePath.trim());
    if (!notePath) {
      throw new Error('Paste an idea brief or provide an idea note path.');
    }

    const file = this.app.vault.getAbstractFileByPath(notePath);
    if (!(file instanceof TFile)) {
      throw new Error(`Idea note not found: ${notePath}`);
    }

    const raw = await this.app.vault.read(file);
    const resolved = stripFrontmatter(raw).trim();
    if (!resolved) {
      throw new Error(`Idea note is empty: ${notePath}`);
    }
    return resolved;
  }
  ...
  private getNormalizedFocusedNotePaths(): string[] {
    return [...new Set(
      this.focusedNotePaths
        .map(path => normalizeVaultPath(path))
        .filter(Boolean)
    )].sort((left, right) => left.localeCompare(right));
  }

  private async collectTargetPages(): Promise<LoreDeltaExistingPageInput[]> {
    const selectedScopes = this.getNormalizedSelectedScopes();
    const focusedPaths = new Set(this.getNormalizedFocusedNotePaths());
    if (selectedScopes.length === 0 && focusedPaths.size === 0) {
      throw new Error('Select at least one lorebook or focused target note.');
    }
    ...
    for (const file of files) {
      ...
      if (focusedPaths.has(normalizedPath)) {
        selected.push(file);
        continue;
      }
    ...
    const pages: LoreDeltaExistingPageInput[] = [];
  ...
  private setIdeaPathFromActiveNote(): void {
    const activeFile = this.app.workspace.getActiveFile();
    if (!(activeFile instanceof TFile)) {
      new Notice('No active markdown note to use as source.');
      return;
    }
    this.sourceIdeaNotePath = activeFile.path;
    this.render();
  }

  private openIdeaNotePicker(): void {
    const files = this.app.vault.getMarkdownFiles();
    if (files.length === 0) {
      new Notice('No markdown notes found.');
      return;
    }
    const modal = new LoreDeltaNotePickerModal(this.app, files, file => {
      this.sourceIdeaNotePath = file.path;
      this.render();
    }, 'Pick an idea note...');
    modal.open();
  }

  private addFocusedNotePath(path: string): void {
    const normalized = normalizeVaultPath(path);
    if (!normalized) {
      return;
    }
    if (this.focusedNotePaths.some(item => normalizeVaultPath(item) === normalized)) {
      return;
    }
    this.focusedNotePaths.push(normalized);
    this.focusedNotePaths.sort((left, right) => left.localeCompare(right));
    this.render();
  }

  private addActiveNoteAsFocusedTarget(): void {
    const activeFile = this.app.workspace.getActiveFile();
    if (!(activeFile instanceof TFile)) {
      new Notice('No active markdown note to add.');
      return;
    }
    this.addFocusedNotePath(activeFile.path);
  }

  private openFocusedNotePicker(): void {
    const files = this.app.vault.getMarkdownFiles();
    if (files.length === 0) {
      new Notice('No markdown notes found.');
      return;
    ...
    const modal = new LoreDeltaNotePickerModal(this.app, files, file => {
      this.addFocusedNotePath(file.path);
    }, 'Pick a focused target note...');
    modal.open();
  }

  private removeFocusedNotePath(path: string): void {
    const normalized = normalizeVaultPath(path);
    this.focusedNotePaths = this.focusedNotePaths
      .filter(item => normalizeVaultPath(item) !== normalized);
    this.render();
  }
  ...
  private getSelectedPages(): LoreDeltaPlannedPage[] {
    if (!this.lastPreview) {
      return [];
    }
  ...
  private getConflicts(): LoreDeltaConflict[] {
    return this.lastPreview?.conflicts ?? [];
  }

  private getConflictDecision(conflictId: string): LoreDeltaConflictDecision {
    return this.conflictDecisions.get(conflictId) ?? 'pending';
  }

  private setConflictDecision(conflictId: string, decision: LoreDeltaConflictDecision): void {
    this.conflictDecisions.set(conflictId, decision);
    this.render();
  }

  private setAllConflictDecisions(decision: LoreDeltaConflictDecision): void {
  ...
  private getConflictCounts(conflicts: LoreDeltaConflict[]): Record<LoreDeltaConflictDecision, number> {
    const counts: Record<LoreDeltaConflictDecision, number> = {
      pending: 0,
      accept: 0,
      reject: 0,
      keep_both: 0
  ...
  private getFilteredConflicts(conflicts: LoreDeltaConflict[]): LoreDeltaConflict[] {
  ...
  private getConflictOutcome(conflictId: string): LoreDeltaConflictOutcome | null {
    return this.conflictOutcomes.get(conflictId) ?? null;
  }

  private getConflictOutcomeLabel(outcome: LoreDeltaConflictOutcome): string {
    if (outcome === 'applied') {
  ...
  private resolveSelectedApplyPages(selected: LoreDeltaPlannedPage[]): ImportedWikiPage[] {
    ...
    const sourcePath = normalizeVaultPath(this.sourceIdeaNotePath.trim());
  ...
  private async runPreview(): Promise<void> {
    ...
    this.progressStage = 'Starting lore delta preview...';
    ...
    try {
      const ideaMarkdown = await this.resolveIdeaMarkdown();
      const existingPages = await this.collectTargetPages();
      this.lastExistingPageCount = existingPages.length;
      const selectedScopes = this.getNormalizedSelectedScopes();
      const focusedPaths = this.getNormalizedFocusedNotePaths();
      ...
      const result = await buildLoreDeltaPlan({
        ...
        allowCreateNotes: this.allowCreateNotes,
        ...
        focusedPagePaths: focusedPaths,
        maxFocusedPagesInPrompt: this.maxFocusedPagesInPrompt,
        maxFocusedPageChars: this.maxFocusedPageChars,
        ...
        callModel: (systemPrompt, userPrompt) => {
          return requestStoryContinuation(completionResolution.completion, {
            systemPrompt,
            userPrompt,
            operationName: 'lore_delta_preview',
            onOperationLog: record => this.plugin.appendCompletionOperationLog(record, {
              costProfile: completionResolution.costProfile
            }),
            onUsage: usage => {
              void this.plugin.recordCompletionUsage('lore_delta_preview', usage, {
                scopeCount: selectedScopes.length,
                scopes: selectedScopes,
                focusedNoteCount: focusedPaths.length,
                focusedNotePaths: focusedPaths,
                sourceIdeaNotePath: this.sourceIdeaNotePath.trim(),
                updatePolicy: this.updatePolicy,
                allowCreateNotes: this.allowCreateNotes,
      ...
      const nextDecisions = new Map<string, LoreDeltaConflictDecision>();
      for (const conflict of result.conflicts) {
        nextDecisions.set(conflict.id, this.conflictDecisions.get(conflict.id) ?? 'pending');
      }
      this.conflictDecisions = nextDecisions;
      this.conflictOutcomes = new Map<string, LoreDeltaConflictOutcome>();
      this.processedChangePaths = new Set<string>();
      this.hideProcessedConflicts = false;
      this.conflictFilter = 'all';
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error('Lore delta preview failed:', error);
      this.lastPreview = null;
      this.conflictDecisions = new Map<string, LoreDeltaConflictDecision>();
      this.previewError = message;
      this.setProgress('Lore delta preview failed', message);
      new Notice(`Lore delta preview failed: ${message}`);
    } finally {
      this.running = false;
  ...
  private async applySelectedPreview(): Promise<void> {
    ...
    if (!this.lastPreview) {
      new Notice('Run preview before applying lore delta updates.');
      return;
    }
    ...
    this.setProgress('Applying selected lore updates...');
    ...
    try {
      ...
      const applied = await applyImportedWikiPages(this.app, pages, {
        onProgress: event => {
          this.setProgress(
            'Applying selected lore updates...',
      ...
      this.setProgress(
        'Lore updates applied',
        `${applied.created} created, ${applied.updated} updated`
      );
      new Notice(`Lore delta applied: ${applied.created} created, ${applied.updated} updated.`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error('Lore delta apply failed:', error);
      this.applyStatus = `Apply failed: ${message}`;
      this.setProgress('Apply failed', message);
      new Notice(`Lore delta apply failed: ${message}`);
    } finally {
      this.running = false;
      this.runningMode = null;
      this.render();
  ...
  private renderChangeItem(
    ...
    change: LoreDeltaPlannedChange,
    page: LoreDeltaPlannedPage | undefined
  ): void {
  ...
  private renderDiffDetails(
    container: HTMLElement,
    label: string,
    diff: LoreDeltaPlannedPage['diff'],
  ...
  private renderConflictReview(
    ...
    pagesByPath: Map<string, LoreDeltaPlannedPage>
  ): void {
    ...
    const filterOptions: Array<{value: LoreDeltaConflictFilter; label: string}> = [
    ...
    filterSelect.addEventListener('change', () => {
      const value = filterSelect.value as LoreDeltaConflictFilter;
  ...
  private renderPreviewOutput(container: HTMLElement): void {
    ...
    if (this.running) {
      container.createEl('p', {
        text: this.runningMode === 'apply'
          ? 'Applying selected lore-delta writes...'
          : 'Running lore-delta preview...'
      });
      return;
    }
    ...
    if (!this.lastPreview) {
      container.createEl('p', {
        text: 'Run preview to inspect deterministic lore-delta changes, approve specific changes, then apply selected writes.'
      });
      return;
    }
    ...
    if (this.lastExistingPageCount > 0) {
      ...
      const focused = this.getNormalizedFocusedNotePaths();
      container.createEl('p', {
        text: `Existing pages considered: ${this.lastExistingPageCount} | Lorebooks: ${scopes.join(', ') || '(none)'} | Focused notes: ${focused.length}`
      });
    }
    ...
    const pagesByPath = new Map<string, LoreDeltaPlannedPage>();
  ...
  private render(): void {
    ...
    contentEl.createEl('h2', { text: 'Apply Lore Delta to Existing Wiki' });

    new Setting(contentEl)
      .setName('Idea Note Path (Optional)')
      .setDesc('Used when Idea Brief is empty.')
      .addText(text => text
        .setPlaceholder('LoreVault/scratch/new-canon-idea.md')
        .setValue(this.sourceIdeaNotePath)
        .onChange(value => {
          this.sourceIdeaNotePath = value.trim();
        }))
      .addButton(button => button
        .setButtonText('Pick Note')
        .onClick(() => {
          this.openIdeaNotePicker();
        }))
      .addButton(button => button
        .setButtonText('Use Active Note')
        .onClick(() => {
          this.setIdeaPathFromActiveNote();
        }));

    new Setting(contentEl)
      .setName('Idea Brief')
      .setDesc('Paste the lore change request, design note, or canon update brief. Leave empty to load from Idea Note Path.')
      .addTextArea(text => {
        text.inputEl.rows = 10;
        text
          .setPlaceholder('A new idea changes multiple faction relationships...\n')
          .setValue(this.ideaMarkdown)
          .onChange(value => {
            this.ideaMarkdown = value;
          });
      });

    new Setting(contentEl)
      .setName('Completion Profile')
      .setDesc('Profile used for model calls in lore delta preview.')
    ...
    const scopesSetting = new Setting(contentEl)
      ...
      .setDesc('These lorebooks provide the candidate pages for lore-delta updates.');
    ...
    const focusedSetting = new Setting(contentEl)
      .setName('Focused Target Notes')
      .setDesc('Focused notes get full-content context in the prompt and can be rewritten when rewrite_focused is enabled.');
    const focusedList = focusedSetting.controlEl.createDiv({ cls: 'lorevault-import-review-list' });
    const focusedPaths = this.getNormalizedFocusedNotePaths();
    if (focusedPaths.length === 0) {
      focusedList.createEl('p', { text: 'No focused notes selected.' });
    } else {
      for (const path of focusedPaths) {
        const row = focusedList.createDiv({ cls: 'lorevault-import-review-item' });
        row.createEl('code', { text: path });
        const removeButton = row.createEl('button', { text: 'Delete' });
        removeButton.addEventListener('click', () => {
          this.removeFocusedNotePath(path);
        });
      }
    }
    const focusedActions = focusedSetting.controlEl.createDiv({ cls: 'lorevault-import-actions' });
    const addFocusedButton = focusedActions.createEl('button', { text: 'Add Target Note' });
    addFocusedButton.addEventListener('click', () => {
      this.openFocusedNotePicker();
    });
    const addActiveFocusedButton = focusedActions.createEl('button', { text: 'Use Active Note' });
    addActiveFocusedButton.addEventListener('click', () => {
      this.addActiveNoteAsFocusedTarget();
    });

    let newNoteTargetFolderInput: { setValue: (value: string) => void } | null = null;
    new Setting(contentEl)
      .setName('New Note Target Folder')
      .setDesc('Used only when Lore Delta creates new notes.')
      .addText(text => {
    ...
    new Setting(contentEl)
      ...
      .setDesc('section_merge merges into matching headings and preserves untouched sections. rewrite_focused additionally allows full managed-body rewrites for focused target notes.')
      .addDropdown(dropdown => {
        dropdown.addOption('section_merge', 'section_merge');
        dropdown.addOption('rewrite_focused', 'rewrite_focused');
        dropdown.setValue(this.updatePolicy);
        dropdown.onChange(value => {
          if (value === 'section_merge' || value === 'rewrite_focused') {
    ...
    new Setting(contentEl)
      .setName('Allow New Notes')
      .setDesc('If disabled, Lore Delta only proposes updates to existing notes.')
      .addToggle(toggle => toggle
        .setValue(this.allowCreateNotes)
        .onChange(value => {
          this.allowCreateNotes = value;
        }));
    ...
    new Setting(contentEl)
      ...
      .setDesc('Deterministic chunk size target for lore delta processing.')
      .addText(text => text
    ...
    new Setting(contentEl)
      ...
      .setDesc('Upper bound on proposed lore-update operations for each chunk.')
      .addText(text => text
    ...
    new Setting(contentEl)
      ...
      .setDesc('Cap on candidate page snapshots injected per chunk.')
      .addText(text => text
    ...
    new Setting(contentEl)
      .setName('Max Focused Notes In Prompt')
      .setDesc('Cap on full-content focused note snapshots injected per chunk.')
      .addText(text => text
        .setValue(this.maxFocusedPagesInPrompt.toString())
        .onChange(value => {
          const parsed = parseInt(value, 10);
          if (!isNaN(parsed) && parsed >= 1) {
            this.maxFocusedPagesInPrompt = parsed;
          }
        }));

    new Setting(contentEl)
      .setName('Focused Note Body Chars')
      .setDesc('Maximum characters of each focused note body included in the prompt.')
      .addText(text => text
        .setValue(this.maxFocusedPageChars.toString())
        .onChange(value => {
          const parsed = parseInt(value, 10);
          if (!isNaN(parsed) && parsed >= 200) {
            this.maxFocusedPageChars = parsed;
          }
        }));
